...
export default function HallForShowing() {
  ...
  const optionPreRes = makeOptions("POST", preReservationBody, undefined, true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [showingData, reservedSeatsData, rowsData] = await Promise.all([
        ...
        <div className="header-title">
          <div className="header-body">
            ...
            {showTotalSeatPrice() > 0 && (
              ...
              >
                <button className="paymentButton">Make reservation</button>
              </Link>
            )}
          </div>
      ...
      <div className="hall-container">
        <div className="hall">
          {[...rows].reverse().map((row) => {
            ...
            return (
              <div key={row.id} className="row" id={"row-" + row.rowNumber}>
                {sortedSeats.map((seat) => (
                  <div
                    className={`seat ${
                      reservedSeats.some(
                        (reservedSeat) =>
                          reservedSeat.seatNumber === seat.seatNumber
                      )
                        ? "reserved"
                        : ""
                    } ${
