...
export default function ReservationsList() {
  ...
  async function getShowing(showingId: number) {
    try {
      const response = await fetch(`${API_URL}/showings/${showingId}`, options);
      const data = await response.json();
      return data;
    } catch (error) {
    ...
    if (confirm) {
      try {
        ...
        if (response.ok) {
          ...
          throw new Error(`Error deleting reservation: ${errorData.message}`);
        }
      } catch (error) {
        console.error(error);
          ...
          <tbody>
            {filteredReservations.map((reservation, index) => (
              <tr key={reservation.id}>
                ...
                <td>
                  <ul className="reservation-seats-list">
                    {reservation.seats.map((seat) => (
                      <li key={seat.id}>Seat: {seat.seatNumber}</li>
                    ))}
                  </ul>
                </td>
                ...
                <td>
                  <button
                    onClick={async () => deleteReservation(reservation.id)}
